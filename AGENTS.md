...
# AGENTS.md — addendum (merge into existing file, do not overwrite)

NOTE: The repo already ships an `AGENTS.md` with the transcript-logging
instructions (log.txt path for macOS/Linux vs Windows). Do NOT replace that
file wholesale — append the section below to it, keeping the existing
logging instructions intact. Losing that section breaks the submission
...
Before making any change, read `SPEC.md` in full. It is the canonical
requirements document for this project, shared across Claude Code, Codex,
and Cursor. Do not infer requirements from `problem_statement.md` alone —
`SPEC.md` is the operationalized, REQ-tagged version of it and takes
precedence if the two ever seem to diverge (they shouldn't; flag it if so).

## Workflow expectations

Work phase-gated per `SPEC.md` §0/§2 (P0 through P5). Don't jump ahead to a
later phase before the current phase's REQ items are implemented.

Inspect the real dataset files before implementing a phase that depends on
them — schema assumptions from prose descriptions are not a substitute for
looking at actual columns/values.

## Phase planning

When asked to plan, scaffold, or generate implementation prompts for a
phase, use `.agents/skills/orchestrate-phase-planner/SKILL.md`.
...
This codebase may be picked up by Claude Code or Cursor in the same session
or a later one (rate-limit fallback). Leave the working tree in a state
where another tool/agent can resume from `SPEC.md` + repo state alone,
without needing this specific conversation's context. Avoid partially-applied
changes across phase boundaries — finish or explicitly stub-and-note a phase
before stopping.

## Secrets

Read API keys from environment variables only. Never write a key to any file
in this repo.