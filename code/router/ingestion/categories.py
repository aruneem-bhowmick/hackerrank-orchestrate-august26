...
"""The fixed coarse taxonomy for media_category on image messages, plus an
"unclassified" catch-all the vision model may return when no bucket
genuinely fits. Never extend this set to match a model's off-taxonomy
output — see validate_image_category."""
...
def validate_image_category(raw: str | None) -> str | None:
    ...
    available," matching this pipeline's rule of never guessing at a
    result it cannot actually stand behind.
    """
